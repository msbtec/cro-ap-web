...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.profissional.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.profissionals.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group {{ $errors->has('tipo_endereco') ? 'has-error' : '' }}">
                ...
                <select id="tipo_endereco" name="tipo_endereco" class="form-control">
                    ...
                    @foreach(App\Profissional::TIPO_ENDERECO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipo_endereco', null) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
