...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.denuncium.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.denuncia.update", [$denuncium->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                ...
                <select id="status" name="status" class="form-control">
                    ...
                    @foreach(App\Denuncium::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $denuncium->status) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
